...
printf("\nThe Client 1 sends the request %s to the Server 1 with port number 21009 and IP address %s\n",get,ip1);
printf("\nThe Client 1's port number is %d and the IP address is %s\n",ntohs(clientaddr2.sin_port),ip1);


inet_ntop(AF_INET,&(clientaddr2.sin_addr),ip1,INET_ADDRSTRLEN);
printf("\n The Client 1 received the value %s from the Server 1 with port number 21009 and IP address %s\n",post,ip1);
printf("\nThe Client 1's port number is %d and the IP address is %s\n",ntohs(clientaddr2.sin_port),ip1);
